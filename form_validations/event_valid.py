...
def validate_event(event_name, host_name, host_insta, chef1_name, chef1_insta, chef2_name, chef2_insta, type, description, location, s_date, e_date):
   ...
   if host_name and len(host_name) > 40:
       res += "Host name" + "\n"
   if host_insta and len(host_insta) > 20:
       res += "Host Instagram" + "\n"
   if not chef1_name or (len(chef1_name) > 20 or len(valid_word(chef1_name))>0):
       res += "Chef 1 name" + "\n"
   if not chef1_insta or (len(chef1_insta) > 20 or len(valid_instagram(chef1_insta))>0):
       res += "Chef 1 Instagram" + "\n"
   if chef2_name and len(chef2_name) > 40:
       res += "Chef 2 name" + "\n"
   if chef2_insta and (len(chef2_insta) > 20 or len(valid_instagram(chef2_insta))>0):
